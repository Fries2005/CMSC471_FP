...
(function () {
  ...
  function renderPanel(song, allSongs, lyricsBySong) {
    ...
    // Header
    document.getElementById("sp-genre").textContent  = song.genre.toUpperCase();
    ...
    statsEl.innerHTML = `
      <div class="sp-stat"><span class="sp-stat-val">${song.tempo ? Math.round(+song.tempo) : "—"}</span><span class="sp-stat-key">BPM</span></div>
      <div class="sp-stat"><span class="sp-stat-val">${song.loudness ? (+song.loudness).toFixed(1) : "—"}</span><span class="sp-stat-key">dB</span></div>
      <div class="sp-stat"><span class="sp-stat-val">${song.valence ? Math.round(+song.valence * 100) : "—"}%</span><span class="sp-stat-key">Positivity</span></div>
      <div class="sp-stat"><span class="sp-stat-val">${song.danceability ? Math.round(+song.danceability * 100) : "—"}%</span><span class="sp-stat-key">Danceability</span></div>
    `;
    ...
    neighbours.forEach(nb => {
      ...
      `;
      // Click neighbour to open its panel
  ...
  function drawRadar(song) {
    ...
    const pts = FEATURES.map((f, i) => {
      const v = Math.min(1, Math.max(0, +song[f.normKey] || 0));
      const a = (i / n) * 2 * Math.PI - Math.PI / 2;
  ...
  function drawBars(song) {
    ...
    FEATURES.forEach(f => {
      ...
      row.innerHTML = `
        ...
        <span class="sp-bar-val">${Math.round(v * 100)}</span>
      `;
  ...
  const GENRE_COLORS = {
    rap: "#f7b731", rock: "#4b7bec", rb: "#26de81",
    pop: "#fd79a8", misc: "#a29bfe", country: "#e17055",
  };
  function genreColor(g) {
    return GENRE_COLORS[g] || "#74b9ff";
  }
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      .sp-empty { font-size: 12px; color: rgba(255,255,255,0.25); }
